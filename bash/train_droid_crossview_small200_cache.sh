...
if [[ -z "${TAG:-}" ]]; then
  if [[ "$TASK" == "cross_view_stage2" ]]; then
    TAG="droid_crossview_small200_cache_stage2"
  else
    TAG="droid_crossview_small200_cache_stage1"
  fi
fi
...
DATASET_META_ROOT="${DATASET_META_ROOT:-/data1/xuehao/datasets/droid_1.0.1_crossview_meta}"
TRAIN_MANIFEST="${TRAIN_MANIFEST:-$DATASET_META_ROOT/meta/episodes_cross_view_train_81_small200.jsonl}"
VAL_MANIFEST="${VAL_MANIFEST:-$DATASET_META_ROOT/meta/episodes_cross_view_val_81_small50.jsonl}"
...
CACHE_ROOT="${CACHE_ROOT:-$DATASET_META_ROOT/cache_crossview_81f_180x320_small200}"
...
count_cache_files() {
  ...
  find "$split_dir" -maxdepth 1 -type f -name '*.pth' | wc -l
}
...
if [[ "$cache_ready" == "0" ]]; then
  ...
  BUILD_CMD+=(--cross_view_use_keyframe_anchor "$CROSS_VIEW_USE_KEYFRAME_ANCHOR")
  BUILD_CMD+=(--num_keyframe_anchors "$NUM_KEYFRAME_ANCHORS")
  BUILD_CMD+=(--keyframe_anchor_dropout "$KEYFRAME_ANCHOR_DROPOUT")
  if [[ -n "$KEYFRAME_ANCHOR_MANIFEST_TRAIN" ]]; then
    BUILD_CMD+=(--keyframe_anchor_manifest_train "$KEYFRAME_ANCHOR_MANIFEST_TRAIN")
  fi
  if [[ -n "$KEYFRAME_ANCHOR_MANIFEST_VAL" ]]; then
    BUILD_CMD+=(--keyframe_anchor_manifest_val "$KEYFRAME_ANCHOR_MANIFEST_VAL")
  fi
  if [[ -n "$KEYFRAME_ANCHOR_IMAGE_ROOT_TRAIN" ]]; then
    BUILD_CMD+=(--keyframe_anchor_image_root_train "$KEYFRAME_ANCHOR_IMAGE_ROOT_TRAIN")
  fi
  if [[ -n "$KEYFRAME_ANCHOR_IMAGE_ROOT_VAL" ]]; then
    BUILD_CMD+=(--keyframe_anchor_image_root_val "$KEYFRAME_ANCHOR_IMAGE_ROOT_VAL")
  fi
