...
def background_consistency(clip_model, preprocess, video_list, device, read_frame):
    ...
    for video_path in tqdm(video_list, disable=get_rank() > 0):
        ...
        sim_per_image = video_sim / (len(image_features) - 1) if len(image_features) > 1 else 0.0
        sim += video_sim
    ...
    if cnt == 0:
        logger.warning(
            "background_consistency: no frame pairs to aggregate (cnt=0). "
            "Likely cause: video_list is empty on this rank (e.g. num_videos < world_size when using multi-GPU), "
            "or all videos on this rank had <= 1 frame. Using sim_per_frame=0.0."
        )
...
def compute_background_consistency(json_dir, device, submodules_list, **kwargs):
    ...
    if get_world_size() > 1:
        ...
        if cnt == 0:
            logger.warning(
                "background_consistency: total cnt=0 after gather. "
                "Likely cause: num_videos < world_size (some ranks had empty video_list). Using 0.0."
            )
