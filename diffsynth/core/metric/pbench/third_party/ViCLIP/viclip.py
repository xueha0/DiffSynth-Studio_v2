...
class ViCLIP(nn.Module):
    ...
    def __init__(self,  tokenizer=None, pretrain=os.path.join(os.path.dirname(os.path.abspath(__file__)), "ViClip-InternVid-10M-FLT.pth"), freeze_text=True):
        ...
        if pretrain:
            ...
            if "text_encoder.positional_embedding" in state_dict:
                pe = state_dict["text_encoder.positional_embedding"]
                print(f"[ViCLIP] 预训练权重中 text_encoder.positional_embedding.shape = {pe.shape} (context_length = {pe.shape[0]})")
