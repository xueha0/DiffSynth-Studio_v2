...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only evaluate the first N frames of each GT/Pred video. Default: evaluate all frames.",
    )
...
def process_video(
    ...
    for view_idx, row in enumerate(grid):
        ...
        # New: only evaluate the first N frames if requested.
        if eval_frames is not None:
            min_frames = min(min_frames, int(eval_frames))
        ...
        for frame_idx in range(min_frames):
            ...
            if "psnr" in metrics:
                state.psnr_sum[view_idx] += peak_signal_noise_ratio(
                    gt_frame,
                    pred_frame,
                    data_range=1.0,
                )
...
def summarize_metrics(
    ...
    for view_idx in range(state.num_views):
        ...
        lpips_value = (
            state.lpips_sum[view_idx] / frames
            if ("lpips" in state.metrics and frames > 0)
            else None
        )
        ...
        view_metrics.append(
            {
                ...
                "frames": frames,
                "video_count": (
                    len(state.fvd_gt_features[view_idx])
                    if "fvd" in state.metrics
                    else None
                ),
            }
    ...
    overall = {
        "psnr": (
            total_psnr_sum / total_frames
            if ("psnr" in state.metrics and total_frames > 0)
            else None
        ),
        "ssim": (
            total_ssim_sum / total_frames
            if ("ssim" in state.metrics and total_frames > 0)
            else None
        ),
        "lpips": (
            total_lpips_sum / total_frames
            if ("lpips" in state.metrics and total_frames > 0)
            else None
        ),
        "mse": total_mse_sum / total_frames if total_frames > 0 else None,
        "fvd": (
            compute_frechet_distance(state.overall_fvd_gt, state.overall_fvd_pred)
            if "fvd" in state.metrics
            else None
        ),
        "frames": total_frames,
...
def evaluate_video_set(
    ...
    return summarize_metrics(
        ...
        fvd_backbone=(
            getattr(fvd_extractor, "__class__", type(None)).__name__
...
def build_fvd_extractor(args: argparse.Namespace):
    ...
    if args.fvd_backbone == "torchvision":
        return TorchvisionFVDExtractor(
            device=args.device,
